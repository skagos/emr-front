    ...
    export default function PatientDetailsPage({ patientId, onBack, onStartNewVisit, onEditVisit }: PatientDetailsPageProps) {
      ...
      const [files, setFiles] = useState<{ id: string; url: string; createdAt: string }[]>([]);
      ...
      const FILES_API = 'http://localhost:8080/api/files';

      useEffect(() => {
        if (!patientId) return;

        const fetchPatient = async () => { /* existing code */ };
        const fetchVisits = async () => { /* existing code */ };

        const fetchFiles = async () => {
          try {
            setLoadingFiles(true);
            const res = await fetch(`${FILES_API}/patient/${patientId}`);
              ...
              <div className="p-8">
                ...
                <div className="mt-8 pt-8 border-t border-gray-200 dark:border-gray-800">
                  ...
                  {loadingFiles ? (
                    ...
                    <p className="text-gray-500 dark:text-gray-400">No files uploaded for this patient.</p>
                  ) : (
                          ...
                          <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-800 p-6 flex-1 w-full">
                            <div className="space-y-4">
                              <div>
                                ...
                                <p className="truncate text-blue-600 dark:text-blue-400">
                                  {file.url}
                                </p>
                              </div>
                              <div>
